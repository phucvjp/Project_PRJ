...
public class CartServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("remove") != null) {
            ...
            if (c != null && !c.getValue().contains(":null")) {
                String[] tokens = c.getValue().split("%");
                for (int i = 0; i < tokens.length; i++) {
                    String token = tokens[i];
                    if (token.split(":")[0].equals(request.getParameter("remove"))) {
                        tokens[i] = "";
                        break;
                    }
                }
                ...
                for (String token : tokens) {
                    ...
                    if (token.length() >= 3 && c.getValue().length() == 0) {
                        log(token);
                        c.setValue(token);
                ...
                d.updateCart(((Account) session.getAttribute("account")).getCUSTOMER_ID(), c.getValue());
                log(c.getValue());
            ...
            session.setAttribute("account", d.checkA(((Account) session.getAttribute("account")).getCUSTOMER_ID()));
